...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    elseif (isset($_POST['bid_amount'])) {
        if (!isset($_SESSION['user_id'])) {
            echo "Pro prihazovani musite byt prihlaseni.";
            exit;
        }
    ...
    <div class="collapse navbar-collapse" id="navbarNav">
      <ul class="navbar-nav ms-auto">
        <?php if (isset($_SESSION['user_id'])): ?>
          <li class="nav-item"><a class="nav-link" href="#">Prihlasen jako <?php echo htmlspecialchars($_SESSION['username']); ?> (<?php echo htmlspecialchars($user_role); ?>)</a></li>
          <li class="nav-item"><a class="nav-link" href="logout.php">Odhlasit se</a></li>
        <?php else: ?>
          <li class="nav-item"><a class="nav-link" href="login.php">Prihlasit se</a></li>
        ...
        <li class="nav-item"><a class="nav-link" href="new_auction.php">Pridat aukci</a></li>
...
<div class="container py-5">
  ...
  <div class="row mb-4">
    ...
    <div class="col-md-6">
      ...
      <p><strong>Startovni cena:</strong> <?php echo number_format($auction['starting_price'], 2, ',', ' '); ?> Kc</p>
      <p><strong>Aktualne nejvyssi prihoz:</strong> <?php echo number_format($current_max, 2, ',', ' '); ?> Kc</p>
      <p><strong>Konec aukce:</strong> <?php echo date('d.m.Y H:i', strtotime($auction['end_time'])); ?></p>
      <p class="text-muted"><strong>Aukci pridal:</strong> <?php echo htmlspecialchars($auction['username']); ?></p>
      ...
      <?php if (isset($_SESSION['user_id'])): ?>
        <?php if (strtotime($auction['end_time']) > time()): ?>
          <form method="post" class="mt-3">
            <div class="mb-3">
              <label for="bid_amount" class="form-label">Vase nabidka (v Kc):</label>
              <input type="number" step="1" min="<?php echo $current_max + 10; ?>" class="form-control" id="bid_amount" name="bid_amount" required>
              <div class="form-text">Minimalni mozna nabidka: <?php echo number_format($current_max + 10, 0, ',', ' '); ?> Kc</div>
            </div>
            <button type="submit" class="btn btn-success">Prihodit</button>
          </form>
        <?php else: ?>
          <div class="alert alert-info mt-3">Tato aukce jiz skoncila.</div>
        <?php endif; ?>
      <?php else: ?>
        <p class="mt-3">Pro prihazovani se musite <a href="login.php">prihlasit</a>.</p>
      <?php endif; ?>
    </div>
  </div>

  <h3>Historie prihozu</h3>
  <?php if ($bids_result->num_rows > 0): ?>
    <ul class="list-group mb-5">
      <?php while ($bid = $bids_result->fetch_assoc()): ?>
        <li class="list-group-item d-flex justify-content-between align-items-center">
          <div><strong><?php echo htmlspecialchars($bid['username']); ?></strong> prihodil <?php echo number_format($bid['amount'], 2, ',', ' '); ?> Kc</div>
          <small class="text-muted"><?php echo date('d.m.Y H:i', strtotime($bid['bid_time'])); ?></small>
        </li>
      <?php endwhile; ?>
    </ul>
  <?php else: ?>
    <p>Pro tuto aukci zatim nejsou zadne prihozy.</p>
  <?php endif; ?>

  <h3 class="mt-5">Komentare</h3>
  ...
  <?php if (isset($_SESSION['user_id'])): ?>
    <form method="post" class="mb-4">
      <div class="mb-3">
        <label for="comment_content" class="form-label">Pridat komentar (max 100 slov)</label>
  ...
  <?php else: ?>
    <p>Pro pridani komentare se <a href="login.php">prihlaste</a>.</p>
  <?php endif; ?>
